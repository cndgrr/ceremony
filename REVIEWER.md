...
1. **The issue's acceptance criteria** — the PR's `Closes #N`, its
   ...
   **At a `Closes #N` head — every same-repo PR carrying criteria, since
   ...
   and it is a request-changes. Quantify over the **nodes**, never over the
   body's matches — a closing keyword that enrolled nothing is the quoting
   case above, and no defect. Read the graph at the head you are approving
   **and after the body's last edit**: the graph is recomputed from the body,
   so a read taken over an earlier one is evidence about that body and not
   about this one (#613).
